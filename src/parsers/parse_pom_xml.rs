...
pub fn parse(file_path: &Path) -> Result<Vec<Task>, String> {
    ...
    let content = fs::read_to_string(file_path)
        .map_err(|e| format!("Error reading pom.xml file: {}", e))?;

    // Parse the XML
    let doc = Document::parse(&content)
        .map_err(|e| format!("Error parsing pom.xml: {}", e))?;
...
fn add_profile_tasks(tasks: &mut Vec<Task>, root: Node, file_path: &Path) -> Result<(), String> {
    ...
    if let Some(profiles_node) = root.children().find(|n| n.has_tag_name("profiles")) {
        ...
        for profile in profiles_node.children().filter(|n| n.has_tag_name("profile")) {
        ...
        if let Some(plugins_node) = build_node.children().find(|n| n.has_tag_name("plugins")) {
            ...
            for plugin in plugins_node.children().filter(|n| n.has_tag_name("plugin")) {
                ...
                if let Some(executions_node) = plugin.children().find(|n| n.has_tag_name("executions")) {
                    for execution in executions_node.children().filter(|n| n.has_tag_name("execution")) {
                        // Get execution ID
                            ...
                            for goal in goals_node.children().filter(|n| n.has_tag_name("goal")) {
                                if let Some(goal_text) = goal.text() {
                                    ...
                                    tasks.push(Task {
                                        ...
                                        description: Some(format!("Maven plugin goal {} (execution: {})", goal_text, exec_id)),
                                        shadowed_by: None,
                                    });
                                }
                            }
